...
internal class BetweenInterpreter : AbstractLanguage
{
    private readonly string _logicOperator;
    private readonly string _specFlag;
    public BetweenInterpreter(JsonElement expression, ApiQueryBuilder builder, string @logicOperator = "and", string specFlag = null!) : base(expression)
    {
        _builder = builder;
    ...
    public override object? Execute()
    {
        if (_expression.ValueKind == JsonValueKind.Object && _expression.EnumerateObject().Any())
        {
            ...
            return _builder.Between(property.Name, property.Value, _logicOperator);
        }
